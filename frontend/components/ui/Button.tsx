...
export default function Button({ children, variant = "primary", href, className = "", onClick, ...props }: ButtonProps) {
  const baseStyles =
  ...
  const variants = {
    primary:
      "bg-[color:var(--accent)] text-black border border-transparent hover:bg-[color:var(--accent-strong)]",
    secondary:
      "bg-transparent text-white/70 border border-white/20 hover:border-white hover:text-white",
    outline:
      "bg-white/5 text-white border border-white/30 hover:border-white hover:bg-white/10",
  };
