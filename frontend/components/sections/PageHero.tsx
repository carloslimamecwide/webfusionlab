  ...
  return (
    <section className="relative overflow-hidden pb-16 pt-24 text-white">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -left-24 top-8 h-72 w-72 rounded-full bg-[radial-gradient(circle_at_center,rgba(214,255,63,0.35),transparent_65%)] blur-[120px]" />
        <div className="absolute right-6 top-20 h-72 w-72 rounded-full bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.16),transparent_60%)] blur-[120px]" />
        <div className="absolute left-0 top-24 h-px w-[70%] bg-gradient-to-r from-transparent via-white/25 to-transparent" />
      ...
      <div className="relative mx-auto flex w-full max-w-6xl flex-col gap-12 px-6 lg:px-10">
        ...
        >
          <motion.div variants={itemVariants} className="space-y-7">
            ...
            <h1 className="text-4xl uppercase leading-[0.95] tracking-tight sm:text-5xl md:text-6xl lg:text-7xl">
              {title}
            </h1>
            <p className="max-w-2xl text-base text-white/65 md:text-lg">{description}</p>
            ...
            {stats.length > 0 ? (
              <div className="grid gap-3 text-[10px] uppercase tracking-[0.35em] text-white/40 sm:grid-cols-3">
                {stats.map((stat) => (
                  <div key={stat.label} className="rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3">
                    <div className="text-white/35">{stat.label}</div>
                    <div className="mt-2 text-sm uppercase tracking-tight text-white">{stat.value}</div>
                    {stat.note ? <div className="mt-2 text-[10px] text-white/35">{stat.note}</div> : null}
                  </div>
                ))}
              </div>
            ) : null}
